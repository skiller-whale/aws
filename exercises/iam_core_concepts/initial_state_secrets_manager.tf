...
resource "aws_secretsmanager_secret" "whale_app_secret" {
  name_prefix             = "${local.app_name}-secrets"
  recovery_window_in_days = 0
}

resource "aws_secretsmanager_secret_version" "whale_app_secret" {
  secret_id = aws_secretsmanager_secret.whale_app_secret.id
